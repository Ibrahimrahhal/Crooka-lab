...
export default (state = initialStatue, action: Action) => {
  ...
  switch (action.type) {
    ...
    case actionTypes.initNewCar:
      ...
      (state.activeAccident as any).activeCarIndex =
        (getActiveAccident(state) as Accident).involvedCars.length - 1;
      stateToSend = state;
      break;
...
function getActiveCar(state: StoreType): Car | undefined {
  return getActiveAccident(state)?.involvedCars[
    (state.activeAccident || {}).activeCarIndex || 0
  ].car;
}
